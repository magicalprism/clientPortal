...
export default function KanbanView({ 
  ...
  const fetchCompanies = useCallback(async () => {
    ...
    try {
      ...
      console.error('[KanbanView] Error fetching companies:', error);
      setCompanies([]);
    }
  ...
  const fetchProjectsForCompany = useCallback(async (companyId) => {
    ...
    try {
      ...
      console.error('[KanbanView] Error fetching projects:', error);
      setProjects([]);
    } finally {
    ...
    const initializeDefaults = async () => {
      ...
      try {
        ...
        if (!error && defaultProject && !defaultProjectSetRef.current) {
          ...
          console.log('[KanbanView] No default project found, user will need to select');
        }
      } catch (error) {
        console.error('[KanbanView] Error initializing defaults:', error);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const handleModalSuccess = useCallback(async (newRecord) => {
    console.log('[KanbanView] Modal success:', newRecord);
...
<Box sx={{ 
  ...
  <Box sx={{ flex: 1, maxWidth: 600 }}>
    <Box >
      ...
      {currentProject && (
        <ProjectKanbanBoard
          key={`kanban-${currentProject.id}-${kanbanMode}-${showCompleted}`} // Force re-mount on key changes
          ...
          onTaskUpdate={() => {
            ...
            console.log('[KanbanView] Task updated in project:', currentProject.id);
          }}
        />
      )}
