...
export const MediaLibraryPicker = ({ open, onClose, onSelect, record, inline = false, multi = false }) => {
  ...
  const loadMedia = async () => {
    ...
    else console.error('❌ Error loading media:', error);
  };
  ...
  const content = (
    <>
      ...
      <Stack direction="row" spacing={2} sx={{ mb: 2, flexWrap: 'wrap' }}>
        <TextField label="Search" value={filters.alt_text} onChange={(e) => handleFilterChange('alt_text', e.target.value)} size="small" />
        <FormControl size="small">
          <InputLabel>Type</InputLabel>
          <Select value={filters.mime_type} onChange={(e) => handleFilterChange('mime_type', e.target.value)} label="Type">
        ...
        <FormControl size="small">
          ...
          <Select value={filters.company_id} onChange={(e) => handleFilterChange('company_id', e.target.value)} label="Company">
        ...
        <FormControl size="small">
          ...
          <Select value={filters.project_id} onChange={(e) => handleFilterChange('project_id', e.target.value)} label="Project">
                    ...
                    <Grid item xs={4} sm={3} md={2} key={media.id}>
                      ...
                      >
                        {isImage ? (
                          ...
                          <Typography variant="caption" color="text.secondary">{media.mime_type}</Typography>
                        )}
                      </Box>
                    </Grid>
                  );
