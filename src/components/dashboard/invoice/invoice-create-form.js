...
const schema = zod
	.object({
		...
		issueDate: zod.date(),
		dueDate: zod.date(),
		contact: zod.string().min(1, "contact is required").max(255),
							...
							<Grid container spacing={3}>
								...
								>
									<Controller
										...
										name="contact"
										render={({ field }) => (
											<FormControl error={Boolean(errors.contact)} fullWidth>
												<InputLabel>contact</InputLabel>
												<OutlinedInput {...field} />
												{errors.contact ? <FormHelperText>{errors.contact.message}</FormHelperText> : null}
											</FormControl>
										)}
									/>
								</Grid>
