...
export function OrdersFilters({ filters = {}, sortDir = "desc" }) {
	const { contact, id, status } = filters;
	...
	const updateSearchParams = React.useCallback(
		(newFilters, newSortDir) => {
			...
			if (newFilters.contact) {
				searchParams.set("contact", newFilters.contact);
			}
	...
	const handlecontactChange = React.useCallback(
		(value) => {
			updateSearchParams({ ...filters, contact: value }, sortDir);
		},
		[updateSearchParams, filters, sortDir]
	);
	...
	const hasFilters = status || id || contact;
...
function contactFilterPopover() {
	...
	return (
		<FilterPopover anchorEl={anchorEl} onClose={onClose} open={open} title="Filter by contact">
			<FormControl>
				<OutlinedInput
					onChange={(event) => {
